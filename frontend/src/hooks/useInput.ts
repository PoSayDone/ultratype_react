...
const useInput = (enabled: boolean, text: string) => {
	...
	function setLetterData(letter: string, isCorrect: boolean = true) {
		const letterDispatch: Dispatch<LetterActions> = useDispatch()
		letterDispatch({type: LetterActionTypes.INCREMENT_TYPED_COUNTER, payload: {letter: letter}})
		if (!isCorrect) {
			letterDispatch({type: LetterActionTypes.INCREMENT_ERROR_COUNTER, payload: {letter: letter}})
		}
	}
